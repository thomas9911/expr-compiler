...
pub extern "C" fn __expr_add_host(lhs: i64, rhs: i64) -> i64 {
    match as_int(lhs).checked_add(as_int(rhs)) {
        Some(value) => new_int(value),
...
pub extern "C" fn __expr_subtract_host(lhs: i64, rhs: i64) -> i64 {
    match as_int(lhs).checked_sub(as_int(rhs)) {
        Some(value) => new_int(value),
        None => runtime_trap("integer overflow"),
    }
}
...
pub extern "C" fn __expr_multiply_host(lhs: i64, rhs: i64) -> i64 {
    match as_int(lhs).checked_mul(as_int(rhs)) {
        Some(value) => new_int(value),
        None => runtime_trap("integer overflow"),
    }
}
...
fn cmp_to_int(lhs: i64, rhs: i64, pred: impl FnOnce(i64, i64) -> bool) -> i64 {
    new_int(i64::from(pred(as_int(lhs), as_int(rhs))))
}
